 ...
 * Version: 2025-01-20-v10-MERGED-EDITABLE
 */

import { app } from "../../scripts/app.js";
import { ComfyWidgets } from "../../scripts/widgets.js";

console.log("[SAM3] ===== VERSION 10 - MERGED EDITABLE POINTS =====");
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (nodeData.name === "SAM3PointCollector") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                container.style.cssText = "position: relative; width: 100%; overflow: hidden; box-sizing: border-box; margin: 0; padding: 0; display: flex; align-items: center; justify-content: center;";
                ...
                // VHS-style: computeLayoutSize allows widget to grow when node is stretched
                widget.computeLayoutSize = function(node) {
                    if (previewNode.canvasWidget.image && !container.hidden) {
                        return {minHeight: 100, maxHeight: undefined, minWidth: 0};
                    }
                    return {minHeight: 0, maxHeight: 0, minWidth: 0};
                };

                // VHS-style: computeSize returns proportional height only on initial fit,
                // otherwise returns small minimum so node resize controls actual height
                widget._fitProportional = false;
                widget.computeSize = (width) => {
                    if (previewNode.canvasWidget.image && !container.hidden) {
                        const aspectRatio = previewNode.canvasWidget.image.height / previewNode.canvasWidget.image.width;
                        let propHeight = (previewNode.size[0] - 20) * aspectRatio + 10;
                        if (!(propHeight > 0)) propHeight = 0;
                        if (widget._fitProportional) {
                            return [width, propHeight];
                        }
                        return [width, 100];
                    }
                    return [width, -4];
                ...
                // VHS-style: draw callback syncs DOM height to fill remaining node space each frame
                const origDraw = widget.draw;
                widget.draw = function(ctx, node, widgetWidth, widgetY, widgetHeight) {
                    if (origDraw) origDraw.apply(this, arguments);
                    if (!previewNode.canvasWidget.image || container.hidden) return;
                    const bottomPadding = 22;
                    let remaining = previewNode.size[1] - widgetY - bottomPadding;
                    if (remaining < 100) remaining = 100;
                    this.computedHeight = remaining;
                    container.style.height = remaining + "px";
                    if (this.parentEl) this.parentEl.style.height = remaining + "px";
                ...
                this.onExecuted = (message) => {
                    console.log("[SAM3] onExecuted called, keys:", Object.keys(message || {}));

                    if (!message) {
                        console.warn("[SAM3] onExecuted: message is null/undefined");
                        return;
                    }

                    // Handle auto-generated points from detection/pose data
                    // MERGE auto-points into the main editable arrays so user can
                    // drag, delete, and add more points interactively (KJNodes style)
                    if (message.auto_positive && message.auto_positive[0]) {
                        try {
                            const autoPts = JSON.parse(message.auto_positive[0]);
                            if (autoPts.length > 0) {
                                // Replace manual positives with auto-generated ones
                                this.canvasWidget.positivePoints = autoPts;
                                console.log("[SAM3] Merged", autoPts.length, "auto positive → editable points");
                            }
                        } catch(e) {
                            console.error("[SAM3] Failed to parse auto_positive:", e);
                        }
                    }
                    if (message.auto_negative && message.auto_negative[0]) {
                        try {
                            const autoPts = JSON.parse(message.auto_negative[0]);
                            if (autoPts.length > 0) {
                                // Replace manual negatives with auto-generated ones
                                this.canvasWidget.negativePoints = autoPts;
                                console.log("[SAM3] Merged", autoPts.length, "auto negative → editable points");
                            }
                        } catch(e) {
                            console.error("[SAM3] Failed to parse auto_negative:", e);
                        }
                    }

                    // Sync merged points to hidden widgets so Python gets them
                    this.updatePoints();
                    ...
                    if (message.bg_image && message.bg_image[0]) {
                        ...
                        img.onload = () => {
                            ...
                            // VHS-style: fit proportionally on first load
                            widget._fitProportional = true;
                    ...
                    // Safety net redraw
                    setTimeout(() => { this.redrawCanvas(); }, 500);
                };
            ...
            // Helper: Calculate overhead height from title bar, visible widgets, inputs/outputs
            nodeType.prototype._getWidgetsOverhead = function() {
                // LiteGraph title bar ~30px, each visible widget ~26px, slot rows ~22px each, padding ~20px
                let overhead = 50; // title bar + base padding
                // Count visible widgets (not hidden/converted)
                if (this.widgets) {
                    for (const w of this.widgets) {
                        if (!w.hidden && !w.type?.includes("converted-widget") && w.type !== "customCanvas") {
                            overhead += 26;
                        }
                    }
                }
                // Count input/output slots
                const inputSlots = this.inputs ? this.inputs.length : 0;
                const outputSlots = this.outputs ? this.outputs.length : 0;
                overhead += Math.max(inputSlots, outputSlots) * 22;
                return overhead;
            };
            ...
            nodeType.prototype.redrawCanvas = function() {
                ...
                // Point radius scales with canvas size
                const pointRadius = Math.max(6, Math.min(canvas.width, canvas.height) * 0.008);

                // Draw positive points (green with white outline + index label)
                for (let idx = 0; idx < positivePoints.length; idx++) {
                    const point = positivePoints[idx];
                    const isHovered = hoveredPoint?.point === point;
                    const r = isHovered ? pointRadius * 1.5 : pointRadius;
                    ctx.beginPath();
                    ...
                    ctx.textAlign = "center";
                    ctx.fillText(idx.toString(), point.x, point.y - r - 4);
                }

                // Draw negative points (red with white outline + index label)
                for (let idx = 0; idx < negativePoints.length; idx++) {
                    const point = negativePoints[idx];
                    const isHovered = hoveredPoint?.point === point;
                    const r = isHovered ? pointRadius * 1.5 : pointRadius;
                    ctx.beginPath();
                    ctx.arc(point.x, point.y, r, 0, 2 * Math.PI);
                ...
                // Info overlay
                if (image) {
                    const posCount = positivePoints.length;
                    const negCount = negativePoints.length;
                    if (posCount + negCount > 0) {
                        const text = `${posCount} pos, ${negCount} neg`;
                        ctx.fillStyle = "rgba(0,0,0,0.7)";
                        ctx.fillRect(5, 5, ctx.measureText(text).width + 20, 22);
                        ctx.fillStyle = "#0ff";
